"""
verify_and_import_15.py
========================
Phase 1: Verify the join assumption (id in JSON == word_id in layout DB)
Phase 2: Full import into the app's word_layout table

Run from: d:\\AL Quran\\al_quran_app\\scratch\\
  python verify_and_import_15.py
"""
...
LAYOUT_DB_PATH = os.path.join(BASE_DIR, "qudratullah-indopak-15-lines.db")
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 1 — Load JSON and build id→text lookup (sorted by true Quran order)
# ══════════════════════════════════════════════════════════════════════════════
print("=" * 70)
print("STEP 1: Loading indopak-nastaleeq.json ...")

with open(JSON_PATH, encoding="utf-8") as f:
    raw = json.load(f)

# Sort by (surah, ayah, word) numerically — do NOT trust dict/JSON key order
...
total_words = len(id_to_text)
min_id      = min(id_to_text)
max_id      = max(id_to_text)
print(f"  Loaded {total_words:,} word entries  (id range: {min_id} - {max_id})")

# ══════════════════════════════════════════════════════════════════════════════
# STEP 2 — Verification: word ids 1-7 should be Bismillah of Al-Fatiha
# ══════════════════════════════════════════════════════════════════════════════
print()
print("=" * 70)
print("STEP 2: Verification -- word ids 1 through 7 (Bismillah of Al-Fatiha)")
print()

VERIFY_IDS = list(range(1, 8))
all_ok = True
for wid in VERIFY_IDS:
...
surah1_ids = [wid for wid in VERIFY_IDS if id_to_meta.get(wid, (0,))[0] == 1]
print()
if not all_ok:
    print("VERIFICATION FAILED -- some word ids were not found in JSON.")
    print("Stopping. Do NOT proceed with bulk import.")
    sys.exit(1)
elif len(surah1_ids) == 0:
    print("VERIFICATION FAILED -- none of ids 1-7 belong to Surah 1.")
    print("The id field does NOT match the layout word_id sequence.")
    print("Stopping. Do NOT proceed with bulk import.")
    sys.exit(1)
else:
    print("VERIFICATION PASSED -- ids 1-7 resolve to Surah Al-Fatiha words.")

# Cross-check with layout DB page 1
print()
print("  Cross-checking layout DB page 1 ...")
...
print(f"  Tables in layout DB: {tables}")

layout_cur.execute("PRAGMA table_info(pages)")
cols = [r[1] for r in layout_cur.fetchall()]
print(f"  Columns in pages: {cols}")

layout_cur.execute("SELECT * FROM pages WHERE page_number=1 ORDER BY line_number")
page1_rows = layout_cur.fetchall()
print(f"  Page 1 has {len(page1_rows)} line rows:")
for row in page1_rows:
    ...
    # Safe ASCII representation of Arabic for Windows console
    ...
    print(f"    line={row['line_number']:2d}  type={ltype:12s}  "
          f"fwid={fwid} [{ft_safe[:20]}]  lwid={lwid} [{lt_safe[:20]}]")

# ══════════════════════════════════════════════════════════════════════════════
# STEP 3-5 — Full import
# ══════════════════════════════════════════════════════════════════════════════
print()
print("=" * 70)
print("Auto-proceeding with FULL import (edit script to disable) ...")
# To run interactively: replace the next line with: ans = input("Proceed? [y/N]: ")
ans = "y"

print()
print("STEP 3-5: Running full import into app database ...")
